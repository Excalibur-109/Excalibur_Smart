...
EXCALIBUR_FORCE_INLINE constexpr Vector<T, 2> CurveLerp(const Vector<T, 2>& start, const Vector<T, 2>& end, T amount) noexcept {
    return {
...
template <FloatingScalar T>
EXCALIBUR_FORCE_INLINE constexpr Vector<T, 3> CurveLerp(const Vector<T, 3>& start, const Vector<T, 3>& end, T amount) noexcept {
    return {
        start.x + (end.x - start.x) * amount,
        start.y + (end.y - start.y) * amount,
...
template <FloatingScalar T>
EXCALIBUR_FORCE_INLINE constexpr Vector<T, 4> CurveLerp(const Vector<T, 4>& start, const Vector<T, 4>& end, T amount) noexcept {
    return {
        start.x + (end.x - start.x) * amount,
        start.y + (end.y - start.y) * amount,
        start.z + (end.z - start.z) * amount,
...
template <typename Value, FloatingScalar T>
EXCALIBUR_FORCE_INLINE constexpr Value EvaluateBezierLevel(Value* level, std::size_t count, T amount) noexcept {
