...
//
// 使用 metal_18 贴图的 Cook-Torrance PBR：
...
layout(set = 0, binding = 7) uniform sampler2D heightTexture;

layout(location = 0) in vec3 vWorldPos;
layout(location = 1) in vec3 vWorldNormal;
layout(location = 2) in vec2 vUV;
layout(location = 3) in vec4 vWorldTangent;
...
// 使用高度图做一次轻量偏移。它比完整 POM 更便宜，也能让 metal_18 的划痕和凹槽
// 在斜视时产生可见层次；Repeat sampler 会自动处理超出 [0, 1] 的坐标。
vec2 parallaxUV(vec2 uv, vec3 viewDirectionTS) {
    float height = texture(heightTexture, uv).r;
    float viewZ = max(abs(viewDirectionTS.z), 0.12);
    float signedHeight = height - 0.5;
    return uv - (viewDirectionTS.xy / viewZ) *
        (signedHeight * ubo.materialParams.z);
}
...
void main() {
    ...
    vec2 materialUV = parallaxUV(vUV, transpose(tangentToWorld) * V);

    vec3 normalTS = texture(normalTexture, materialUV).xyz * 2.0 - 1.0;
    vec3 N = normalize(tangentToWorld * normalTS);
    ...
    // baseColorTexture 是 RGBA8_SRGB，硬件采样后已得到线性 albedo；其余数据贴图
    // 使用 RGBA8_UNorm，R 通道分别保存 metallic、roughness 和 height。
    vec3  albedo    = texture(baseColorTexture, materialUV).rgb * ubo.baseColor.rgb;
    float metallic  = clamp(texture(metallicTexture, materialUV).r * ubo.baseColor.a, 0.0, 1.0);
    float roughness = clamp(texture(roughnessTexture, materialUV).r * ubo.materialParams.x, 0.045, 1.0);
    ...
    vec3 environmentDiffuse = texture(
        skyboxTexture,
        rotateSkyDirectionY(N, ubo.materialParams.w)).rgb;
    vec3 reflectionDirection = reflect(-V, N);
    vec3 environmentSpecular = texture(
        skyboxTexture,
        rotateSkyDirectionY(reflectionDirection, ubo.materialParams.w)).rgb;
