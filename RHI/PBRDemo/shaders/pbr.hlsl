...
cbuffer PBRConstants : register(b0) {
    ...
    float4 baseColor;
    // x=roughness multiplier, y=AO, z=height-map parallax scale, w=skybox Y rotation in radians.
...
Texture2D<float4> roughnessTexture : register(t6);
SamplerState roughnessSampler : register(s6);
Texture2D<float4> heightTexture : register(t7);
SamplerState heightSampler : register(s7);
...
VertexOutput VSMain(VertexInput input) {
    ...
    output.worldTangent = float4(
        normalize(mul((float3x3)model, input.tangent.xyz)),
        input.tangent.w);
    return output;
}
...
float2 ParallaxUV(float2 uv, float3 viewDirectionTS) {
    const float height = heightTexture.Sample(heightSampler, uv).r;
    const float viewZ = max(abs(viewDirectionTS.z), 0.12F);
    const float signedHeight = height - 0.5F;
    return uv - (viewDirectionTS.xy / viewZ) *
        (signedHeight * materialParameters.z);
}
...
float4 PSMain(VertexOutput input) : SV_TARGET0 {
    ...
    const float3 bitangent = normalize(cross(geometricNormal, tangent)) *
        input.worldTangent.w;
    const float3 viewDirectionTS = float3(
        dot(V, tangent),
        dot(V, bitangent),
        dot(V, geometricNormal));
    const float2 materialUV = ParallaxUV(input.uv, viewDirectionTS);
    const float3 normalTS = normalTexture.Sample(normalSampler, materialUV).xyz *
        2.0F - 1.0F;
    const float3 N = normalize(
        tangent * normalTS.x + bitangent * normalTS.y + geometricNormal * normalTS.z);
    ...
    const float3 albedo = baseColorTexture.Sample(baseColorSampler, materialUV).rgb *
        baseColor.rgb;
    const float metallic = clamp(
        metallicTexture.Sample(metallicSampler, materialUV).r * baseColor.a,
        0.0F,
        1.0F);
    const float roughness = clamp(
        roughnessTexture.Sample(roughnessSampler, materialUV).r * materialParameters.x,
        0.045F,
        1.0F);
    ...
    const float3 environmentDiffuse = skyboxTexture.Sample(
        skyboxSampler,
        RotateSkyDirectionY(N, materialParameters.w)).rgb;
    const float3 environmentSpecular = skyboxTexture.Sample(
        skyboxSampler,
        RotateSkyDirectionY(reflect(-V, N), materialParameters.w)).rgb;
